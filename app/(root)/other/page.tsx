...
function OutdoorPage() {
  const officeData = servicebarLinks.find((link) => link.name === "Other");
  if (!officeData) {
    return <div>Error: Other data not found!</div>;
  }
  ...
  const handleSendEmail = () => {
    const mailtoLink = `mailto:vnick8@hotmail.com?subject=Other Renovation&body=Name: ${formData.name}%0D%0A%0D%0APhone Number: ${formData.phoneNumber}%0D%0A%0D%0AAddress: ${formData.address}%0D%0A%0D%0ADescription: ${formData.description}`;
  ...
  return (
    <div className="bg-gradient-to-r from-gray-100 to-green-300/30 min-h-screen flex flex-col items-center justify-center py-12">
      <h1 className="text-4xl font-bold mb-8 text-black">Other</h1>
